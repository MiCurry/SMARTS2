...
class inverse_mf_openmpi:
    test_name = "Inverse Matrix-Free ModEM Test with OpenMPI"
    test_description = "Test inverse for all compiled versions of 3D Matrix-Free with OpenMPI (2 NLCG Itrs)"
    dependencies = None
    ncpus = 7
    ...
    def run(self, env, result, src_dir, test_dir, hpc=None, *args, **kwargs):
        ...
        for exe in executables:
            ...
            print(f"Running OpenMPI MF inverse: '{exe_launch}' logging to '{os.path.abspath(log_fname)}'")
            try:
            ...
            print(f"Passed - Inverse succsfully ran {exe} for OpenMPI MF!\n")

            os.chdir('..')


        result.result = "PASSED"
        result.msg = "Successfully ran Matrix-Free inverse with OpenMPI"
