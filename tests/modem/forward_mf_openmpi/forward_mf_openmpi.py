...
class forward_mf_openmpi:
    test_name = "Forward Matrix-Free ModEM Test with OpenMPI"
    test_description = "Test forward for all compiled versions of 3D Matrix Free with OpenMPI"
    dependencies = None
    ncpus = 4
    ...
    def run(self, env, result, src_dir, test_dir, hpc=None, *args, **kwargs):
        ...
        if MODSET_NAME not in gnu_modsets:
            result.result = "FAILED"
            result.msg = f"Could not find a '{MODSET_NAME}' Modset - need an MPICH modset for this test "
            return result.result
        ...
        for exe in executables:
            ...
            print(f"Running OpenMPI MF forward: '{exe_launch}' logging to '{os.path.abspath(log_fname)}'")
            try:
        ...
        result.msg = "Successfully ran matrix-free forward with OpenMPI"
