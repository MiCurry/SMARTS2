...
class forward_sp_mpich:
    test_name = "Forward Sparse-Matrix ModEM Test with MPICH"
    test_description = "Test forward for all compiled versions of 3D Sparse Matrix with MPICH"
    dependencies = None
    ncpus = 4
    ...
    def run(self, env, result, src_dir, test_dir, hpc=None, *args, **kwargs):
        ...
        for exe in executables:
            ...
            print(f"Running MPICH SP forward: '{exe_launch}' logging to '{os.path.abspath(log_fname)}'")
            try:
        ...
        result.msg = "Successfully ran Sparse-Matrix (SP) forward with MPICH"
