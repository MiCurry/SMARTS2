...
class forward_sp_openmpi:
    test_name = "Forward Sparse-Matrix ModEM Test with OpenMPI"
    test_description = "Test forward for all compiled versions of 3D Sparse Matrix with OpenMPI"
    dependencies = None
    ncpus = 4
    ...
    def run(self, env, result, src_dir, test_dir, hpc=None, *args, **kwargs):
        ...
        if MODSET_NAME not in gnu_modsets:
            result.result = "FAILED"
            result.msg = "Could not find a 'GNU-OPENMPI' Modset - need an GNU-OPENMPI modset for this test "
            return result.result
        ...
        for exe in executables:
            ...
            print(f"Running OPENMPI SP forward: '{exe_launch}' logging to '{os.path.abspath(log_fname)}'")
            try:
        ...
        result.msg = "Successfully ran Sparse-Matrix (SP) forward with OpenMPI"
