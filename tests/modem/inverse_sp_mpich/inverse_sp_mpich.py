...
class inverse_sp_mpich:
    test_name = "Inverse Sparse-Matrix ModEM Test with MPICH"
    test_description = "Test inverse for all compiled versions of 3D Sparse Matrix with MPICH (2 NLCG Itrs)"
    dependencies = None
    ncpus = 7
    ...
    def run(self, env, result, src_dir, test_dir, hpc=None, *args, **kwargs):
        ...
        for exe in executables:
            ...
            print(f"Running MPICH SP inverse: '{exe_launch}' logging to '{os.path.abspath(log_fname)}'")
            try:
            ...
            print(f"Passed - Inverse succsfully ran {exe} for MPICH SP!\n")
